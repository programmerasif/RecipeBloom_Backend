...
const CreateRecipeValidationSchema = z.object({
  body: z
    .object({
      ...
      Directions: z.string().nonempty("Directions are required"),
      ...
      disLikes: z.array(z.string().refine((val) => Types.ObjectId.isValid(val), {
        message: "Invalid user ID",
      })).default([]),
      ...
      rating: z.array(z.object({
        ...
        ratingNumber: z.number().min(1).max(5, "Rating must be between 1 and 5").default(1),
      })).default([]),
      totalAverageRating: z.number().min(1, "Average rating must be at least 1").default(1),
